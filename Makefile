...
lint: ## Check and fix code base according to Plone standards
	...
	@uvx pyroma@latest -d .
	@uvx check-python-versions@latest --only pyproject.toml .
...
i18n: $(VENV_FOLDER) ## Update locales
	...
	@$(BIN_FOLDER)/python -m collective.venue.locales
...
test-coverage: $(VENV_FOLDER) ## run tests with coverage
	@$(BIN_FOLDER)/pytest --cov=collective.venue --cov-report term-missing

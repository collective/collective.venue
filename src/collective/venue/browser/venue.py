...
class VenueView(BrowserView):
    def __init__(self, context, request):
        ...
        # If it's not available, it won't hurt.
    ...
    def map_configuration(self):
        ...
        config = {
            ...
            "map_layers": [{"title": _(it), "id": it} for it in map_layers],
        }
    ...
    def data_coordinates(self):
        ...
        return (geo.geolocation.latitude, geo.geolocation.longitude)
    ...
    def data(self):
        ...
        if acc:
            ...
            address["country"] = get_pycountry_name(acc.country) or ""
            address["notes"] = acc.notes.output if acc.notes else ""
        data["address"] = {key: value for key, value in address.items() if value}
        ...
        if acc:
            ...
            contact["fax"] = acc.fax
        data["contact"] = {key: value for key, value in contact.items() if value}
        ...
        data["social"] = {key: value for key, value in social.items() if value}
